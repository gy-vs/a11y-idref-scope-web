...
import {analyzeSnapshotHttp, demoSnapshot} from './analyze-service';
...
export function createApp(){
  ...
  // Built-in captured page covering duplicate ids, token lists, missing
  // targets, cross-iframe and open/closed shadow scopes.
  app.get('/api/snapshot',(_req,res)=>res.json(demoSnapshot()));
  // IDREF analysis. The server resolves targets to stable sids + paths and
  // keeps a revision-keyed scope cache; the frontend only replays the result.
  app.post('/api/snapshot/analyze',(req,res)=>{const out=analyzeSnapshotHttp(req.body);res.status(out.status).json(out.body)});
  return app;
}
